#!/usr/bin/env python3
"""
build_jul06.py — one-shot builder for the Jul 6, 2026 weekly-scan pages.

Differs from generate_release_pages.py in one way: the site nav is now single-source
(data/nav.json + tools/build-nav.js), so this script does NOT use pb.inject_nav().
It slices the generated <!-- NAV:START -->…<!-- NAV:END --> region verbatim from the
template so new pages carry the marker-wrapped nav; nav links for the new pages are
added to data/nav.json and propagated by `node tools/build-nav.js` afterwards.

Local only — nothing is committed or pushed. Mo reviews, then pushes.
"""
import os, datetime
import page_builder as pb
import content_jul2026 as content
...
NEW_SITEMAP_URLS = [
    ("topps-chrome-updates-basketball", 0.85),
    ("topps-chrome-black-football", 0.85),
]
...
if __name__ == "__main__":
    print("== Building Jul 6 release pages ==")
