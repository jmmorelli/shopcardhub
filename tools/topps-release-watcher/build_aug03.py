#!/usr/bin/env python3
"""
build_aug03.py — one-shot builder for the Aug 3, 2026 weekly-scan page:
2026 Bowman Chrome Baseball (/bowman-chrome-baseball-2026).

Same approach as build_jul13.py: slices the <!-- NAV:START -->…<!-- NAV:END --> region
verbatim from the template (nav is single-source via data/nav.json + tools/build-nav.js).
The nav link is added to data/nav.json and propagated with `node tools/build-nav.js`
afterwards. Local only — nothing is committed or pushed. Mo reviews, then pushes.
"""
import os, datetime
import page_builder as pb
import content_aug03 as content
...
NEW_SITEMAP_URLS = [
    ("bowman-chrome-baseball-2026", 0.85),
]
...
if __name__ == "__main__":
    print("== Building Aug 3 release page ==")
    ...
    print("Done. Add nav link to data/nav.json + run `node tools/build-nav.js`, review, then push.")
