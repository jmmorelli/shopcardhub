...
/* ---------- 8. rail-single-source ---------- */
{
  ...
  try { for (const m of read("data/nav.json").matchAll(/"href"\s*:\s*"([^"]+)"/g)) navHrefs.add(m[1].split("#")[0]); } catch {}
  for (const f of pages) {
    const { raw } = P.get(f);
    ...
    for (const hm of m[1].matchAll(/\bhref="([^"]*)"/g)) {
      const h = hm[1].split("#")[0].split("?")[0].replace(/\/$/, "") || "/";
      if (!h.startsWith("/")) add("FAIL", "rail-single-source", f, `rail href ${hm[1]} is not a site path`);
      else if (!navHrefs.has(h)) add("FAIL", "rail-single-source", f, `rail href ${h} is not in data/nav.json (nor /, /watchlist, /indices, /auctions)`);
    }
  }
  if (rails.size > 1) {
