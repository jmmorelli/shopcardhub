...
| **Phone dock for the buy strip** (`44a61ea`, Mo: "fix what's left") — a 390 px scan of all 80 sitemap pages put the strip's top at 700–1080 px on ~40 of them (under the hero paragraph). `js/buy-strip.js` + `site-fixes.css` §8: while the strip's slot is below the viewport it rides the bottom edge as a compact bar (title · live ask · **Buy on eBay →**, z 190 under the mobile nav), and returns to the flow once its slot is in view. No markup change on the 61 strip pages; desktop untouched. Verified live: football / bangers / chrome-baseball / messi dock at load (button at y 785–844), undock by scroll 400. | **LIVE** |
| **Bing Webmaster read** (signed in via the shopcardhub@gmail.com Google session — no password; bing.com/webmasters → Sign In → Google). Data from Sep 8. Clicks/impressions halved on Sep 19 in step with sessions. **Demand, not ranking:** `pokemon 30th anniversary set release date` 109 impressions over the window → 3 in the last 7 days; `topps definitive basketball` 65 → 6; `2026 bowman chrome` 133 → 21; positions unchanged (5–7). Bing's one recommendation: more inbound links from quality domains. Full series in the read doc. | **read** |
| **CHASE strip on all eight Pokémon index pages** (`33ed846`, Mo: "do that for the pokemon pages now" — high-ticket cards in the first buy position; **no COMC on the site**, Mo: "it's passive, I want the site actively earning"). `tools/build-chase-strip.mjs` bakes the top 3 constituents by dated sold mark ($50 floor) after the stats, before the Tuesday Tape + SEALED row, one EPN link each (`customid=<tk>-<num>-chase`, photo key reused from the table row, `chase_click` event); inline script re-reads CARDS on load and `remark-indices.mjs` re-runs the builder after a Monday write. The sector template (`build-sector-index.mjs`) carries the same strip: TH26 = the Mew RGB trio at $3,675–$4,000 (Authenticity Guarantee links), SV151 = Charizard/Blastoise/Venusaur ex. Live-verified on production with real listing photos. **Read the `-chase` rows at the Sep 30 desk beside `home-*`.** | **LIVE** |
