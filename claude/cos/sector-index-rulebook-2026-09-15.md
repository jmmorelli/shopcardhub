...
## Amendment 2 — Bowman family (Sep 25 2026, Mo)

Bowman uses the same rulebook with the universe defined by **card class, not set**: a ticker's universe is every **1st Bowman Chrome Auto** in scope on SCP's console listing, and its sub-index is the same scope's **1st Bowman Chrome base cards** (screened, price-weighted, uncapped, base 100). Scopes: **BOW26** = every 2026 Bowman release (May Bowman, September Bowman Chrome, December Bowman Draft — a new release enters as a third source the first Monday after its console lists, a logged divisor adjustment, level unchanged); **BB26 / BCB26 / BD26** = one release each. **BOW27** starts with May 2027. Parallels may become a third line once a season of solds shows which colours trade. A 1st flag comes from the release checklist in `data/sets/` where one exists; where none does (May 2026), every Chrome Prospect Autograph / Chrome Prospect slot of that release counts as a 1st until a checklist says otherwise. The Bangers board is not an index and stays a ranked board of ten with calls; its benchmark line cites BB26.
