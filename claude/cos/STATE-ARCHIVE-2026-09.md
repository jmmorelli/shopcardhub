# CoS state — ARCHIVE, 2026-09 (everything before the Sep 22 split)

> Moved verbatim out of `claude/cos/STATE.md` on 2026-09-22. **Not read at STEP 0.** Grep it for the history
> behind a rule. Where this file and STATE.md disagree, STATE.md is current.
...
- **P0 (Wednesday build, top of the queue) — eBay/EPN compliance hardening (Mo, Sep 21 evening: "I do not want to lose eBay EPN — it's our only source of income"):** (1) `/api/comps` and `/api/auctions` are open proxies on Mo's keyset — add a same-origin/referer check, a per-IP rate limit, a hard cap on `limit`, and keep the CDN cache; abuse of an open endpoint reads to eBay as *our* application misbehaving. (2) Stop republishing raw eBay content publicly: `listings-history.json` and the feed carry listing titles/prices/URLs in a public branch — keep derived numbers (marks, medians, counts, levels) public, move raw listing snapshots behind our own origin or drop them past a short window; the feed-origin + repo-private work is the same change. (3) Read the CURRENT eBay API License Agreement and the EPN Network Agreement line by line and write `claude/cos/compliance-posture.md` — what we call, what we store, for how long, what we display, what we never do — and have the desk keep it current; no clause is quoted from memory. (4) PriceCharting: the Pokémon re-marks, SV151 screen and football solds are page reads (scraping; 429s already seen) — the licensed path is PriceCharting's paid API (`api/cron-snapshot.js` was written for it). **No spend below M0**; revisit at the first weekly read that clears $300/mo trailing. eBay Marketplace Insights (sold data, limited access) is a later application, not now. The nightly engine itself contains no LLM and uses eBay's own Browse-API-plus-EPN-header pattern — that part is not the gray area.
...
- **Sep 22 ~13:30 PT (CoS, Mac-linked, Mo in chat: "eight proposals filed awaiting-cos") — the Tuesday board lane's eight filings, ruled.** Applied: `bb-…-terminal-api-contract-fp` (audit-terminal reads a concatenated URL across `+` continuation lines; the standing FAIL on `pokemon-30th-anniversary-2026.html:1011` clears; negative-tested), `bb-…-psa10-correction` (page/feed/tape already fixed in `a520a87`; vendor correction is the CoS desk's 14:00 send — the X Desk Watch's three-point note, incl. "read `asOf` before posting the Tuesday board"; not a Mo item), `bb-…-home-single-link-guard` (guard stays; real fix = idea #30 Wednesday). Approved for the Wednesday build: `bb-…-graded-source-rule` (**standing rule 1 amended: a graded figure needs ONE tape row carrying date + grade + price; never a guide cell, never a date borrowed from another grade's row**), `bb-…-xboard-signal-source` (Verdict line outranks the Signal stat; engine ask-side word never enters `currentSignal`), `bb-…-stale-price-stamps` (re-read murakami + wnba + wembanyama, then restamp). Declined: `bb-…-pierce-watch-tier` (stays tracked — published footer promise, Dec grade needs a continuous mark, dropping losers is survivorship bias), `bb-…-coach-note-stale` (**coachNotes retired as a weekly duty**; ad hoc from the CoS desk only; stale `weekOf` is no longer a finding). **Gate baseline now 0/62 · 0/3 · 0/1** (audit-prices +2 = the two named stale stamps).

- **Sep 21 ~15:30 PT (CoS, Mac-linked, Mo in chat) — THE MONDAY LANE'S FILING EXECUTED, AND THE "WHO READS THE FILINGS" HOLE CLOSED.** Mo pasted `weekly-scan-2026-09-21.md` and asked why he had to. **Answer, on the record: nobody read the Mac-side filings.** Desktop lanes file into `Card Hub/` on the Mac; the CoS runs were cloud tasks with no Mac; the two halves only met through the repo (STATE, pipeline.json) or a chat with Mo. The X Desk Watch's vendor brief "for the CoS to deliver" sat in `x-desk/` for the same reason. **Fixed:** cloud task `trig_018BNFLuPJZENTotAg78Z6bq` **CoS · desk**, weekdays 14:00 PT, bound to Mo's Mac — reads every inbox (Project docs + the Mac folder + `awaiting-cos`), rules, applies auto-approved fixes from a fresh deploy-key clone behind the gates, delivers vendor briefs via the Grok Bot app, carries the old daily's instrument checks and ideas ruling, writes this log and mirrors both copies. **The 06:00 cloud daily is deleted** (Mo had set it to Mondays-only at 14:53 PT while trying to edit the desktop CoS task — the two share a name; the desk replaces it). First fire **Tue Sep 22 14:00 PT** — its folder/device binding is the thing to verify; if it reports "MAC UNREACHABLE" the fix is the task's folder grant, not a new lane. Queued, denied by the safety layer this session: X Desk Watch 17:30 → 13:00 so the desk acts on a vendor FAIL the same day. **The filing itself, executed `2378ba7`:** (1) `audit-site` cut eBay URLs at `)` — the Gonzales OR-syntax query `(gonzalez,gonzales)` was truncated before campid/mkevt/customid, three false FAILs held `card-justin-gonzalez.html` on the gonzalez-only query (3 of 30 titles); class widened in both matches, negative-tested, page regenerated, **live** with the dual query. (2) `tools/remark-indices.mjs` now accepts PF25 (in POKE; the inception-shape `levelchg` and `Since Inception` cells) — dry-run passes 36/36; **PF25's first real re-mark is the Wednesday weekly's** (lane figure: basket $1,150.42 → 95.62). (3) R12/R13/R14 + robots.txt were applied by `b50d54c` before this run; the staged file is gone. (4) Topps calendar bot wall → `releases.json` refresh moves to the desk (Mac Chrome) — queued for the Wednesday prompt pass. (5) Monday prompt still carries the R1-violating lock sweep and no freshness header — Mo's edit, text to be handed to him. (6) Coach note not refreshed — Sunday lane. `greninja-psa10` finalizable since Sep 1 — Wednesday. Gates 0/60 · 0/3 · 0/6.

- **Sep 21 ~15:00 PT (CoS, Mac-linked, Mo in chat) — IDEAS DESK: "anything worth implementing now?" Measured, ruled, one thing started.** **#35 (complete-set links) → PARKED, was adopted this morning:** re-screened with a real `SET_OK`/`SET_BAD` guard on production `/api/comps` — BCB26 **0 clean** (all "complete your set" singles), BOW26 real but **$7–$22 base sets** (~$0.45 a sale, not the desk's $240–$500), AH26 three real sets $36–$280 (and Pokémon comps need `category_ids=183454`; the default returns nothing). Revive on SV151, where the set IS the index and clears $2,000. The desk's screen quoted raw hits; the guard is what decides — same lesson as #32's "CASE BREAK" trap. **#34 (delta chip) → stays Wednesday** (worth ~$0 until #30's cards are in the rail). **#30 (star-page engine cards, Mo-approved Sep 19) → STARTED `b84272e`:** 3 of 8 in the nightly engine tonight — Ohtani 2018 Update #US285 (35 verified asks $137.99+), Wembanyama 2023-24 Prizm #136 base (31, $75+), Yamal 2023-24 Topps Chrome UCC #64 (10, $44.50+) — each run through `verifyListings()` against live listings; **no slug** (both block generators hardcode "Bowman" for any non-TCG card and now refuse cardType `single` until `facts()` reads the set off the label — a slug today would publish a false set name). **Finding for Wednesday, on the record:** the two pages that actually paid cannot be engine pages — Messi Megacracks #71 has 1 raw fixed-price listing (the market is graded, which the engine excludes by design; the $860 sale was never an engine-shaped click) and Ronaldo Mega Craques #137 has 0; PCA and Misiorowski 1st autos are auction-only (0 verified); Haaland has 1 ask; Flagg #251 is an R8 question ("Topps Chrome" vs "Topps Chrome Update" titles) before any query. The Wednesday build therefore: teach `facts()` the set for `single`, slug the three after marks, and treat Messi/Ronaldo/Misiorowski as graded-link pages (#31/#33) — and the 4-of-8 floor in #30's kill criterion is already in doubt; say so at the Oct 21 read, do not lower it. Gates 0/60 · 0/3 · 0/6 (5 = the x-board legacy grace from the parallel `x-board-feed-integrity` session `b50d54c`, which landed four minutes after my `d57f6cc` and extends it; the Tuesday lane's rebuild clears it). Ledger rows 30/34/35 updated in the Project and mirrored here.
